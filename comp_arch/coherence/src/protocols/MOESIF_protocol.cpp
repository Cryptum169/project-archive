...
void MOESIF_protocol::process_snoop_request (Mreq *request)
{
    ...
    case MOESIF_CACHE_FM:
        ...
        break;
    default:
        fatal_error("Invalid Cache Snoop State for MOESIF Protocol\n");
    }
}
...
inline void MOESIF_protocol::do_cache_O (Mreq *request)
{
    ...
    case LOAD:
        ...
        break;
    case STORE:
        // since infinite size cache
        send_GETM(request->addr);
        state = MOESIF_CACHE_OM;
        
        // Others want it, but don't have it, only this have it,
        // therefore miss, coherence miss?
        Sim->cache_misses++;
        break;
    default:
        request->print_msg(my_table->moduleID, "ERROR");
